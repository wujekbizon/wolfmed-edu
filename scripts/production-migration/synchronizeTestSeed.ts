...
export async function synchronizeTestSeed(
  ...
  return sql.begin(async (transaction) => {
    ...
    for (let index = 0; index < staged.length; index += BATCH_SIZE) {
      ...
      await transaction`INSERT INTO migration_test_seed
    ...
    const [before] = await transaction`SELECT
      ...
      (SELECT COUNT(*)::int FROM wolfmed_tests target WHERE NOT EXISTS (
        SELECT 1 FROM migration_test_seed source WHERE source.id = target.id
      )) AS extras
      FROM migration_test_seed source
      LEFT JOIN wolfmed_tests target ON target.id = source.id`

    if (execute) {
      await transaction`INSERT INTO wolfmed_tests
        (id, meta, data, "createdAt", "updatedAt")
        SELECT id, meta, data, created_at, updated_at FROM migration_test_seed
        ON CONFLICT (id) DO UPDATE SET
          meta = excluded.meta, data = excluded.data,
    ...
    const [after] = await transaction`SELECT
      ...
      COUNT(*) FILTER (WHERE EXISTS (
        SELECT 1 FROM wolfmed_tests target WHERE target.id = source.id
          AND (target.meta IS DISTINCT FROM source.meta OR
               target.data IS DISTINCT FROM source.data)
      ))::int AS mismatched
      FROM migration_test_seed source`
