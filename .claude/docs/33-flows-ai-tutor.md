...
5. Calls `executeToolWithContent(toolName, toolDefinition, { request, content, pdfFiles, overrideArgs })` (`src/server/vertex-rag/index.ts` or `src/server/tools/executor.ts`) — the actual model call.
