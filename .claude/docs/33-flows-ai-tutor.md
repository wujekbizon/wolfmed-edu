...
4. `ready` is answered from typed memory alone; `empty`/`unavailable` return fixed responses. Explicit topics resolve against user metadata and filter both facts and episodes before retrieval; broad overview questions enumerate up to 40 active facts. Shared assembly caps text, preserves dates and separates aggregates from individual attempts. Answers address the requested topic and cannot invent detailed weaknesses from percentages alone. Six validated recent turns support self-state continuity without entering medical retrieval.
...
1. **Memory assembly, both halves, both fail-safe** (return `''` if memory is unavailable rather than erroring the whole answer):
   ...
   - `buildMemoryTail(userId, question)` — topic-filtered or hybrid-ranked facts + episodes. Newest-episode fallback only applies when no topic resolves and no episode was found. `retrieveMemory` embeds the memory search query once and fans out to both stores. Normalized lexical fusion preserves the vector baseline; literal matches survive assembly. See [accuracy correction and smoke tests](./47-memory-retrieval-smoke-test.md).
...
4. `generateGroundedAnswer(question, context, { userContext, memoryTail, memoryPrefix })` (`src/server/vertex-rag/generate.ts`) — the actual grounded generation call, with memory folded in as personalization/continuity context. It never enters subject retrieval and cannot supply medical claims — per Data Sources tier 4.
...
6. Trace order follows the turn: `user_msg` before classification/model work, `memory_retrieval`/`rag_retrieval` after recall, then `model_msg` after response. Retrieval traces retain returned fact/episode IDs, scores, tiers, and each store's retrieval mode. The model row combines semantic-router and answer-call usage into `tokenCost`; its payload keeps input/output/thought/total tokens. Events share `(userId, runId, turnIndex)` and expire after 90 days.
