...
1. **Memory assembly, both halves, both fail-safe** (return `''` if memory is unavailable rather than erroring the whole answer):
   ...
   - `buildMemoryTail(userId, question)` — hybrid-ranked facts + episodes relevant to this question, plus a newest-episode continuity fallback. `retrieveMemory` embeds the query once and fans out to both typed stores in parallel. This is the volatile tail (Path B memory).
