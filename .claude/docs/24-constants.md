...
| `memoryMessages.ts` | `EMPTY_SELF_STATE_MESSAGE`, `UNAVAILABLE_SELF_STATE_MESSAGE`, `AMBIGUOUS_TUTOR_INTENT_MESSAGE` | Fixed user-facing outcomes when typed memory is empty/unavailable or semantic intent needs clarification; none may fall through to curriculum RAG. |
