...
| [`41-stripe-payment-plan.md`](./41-stripe-payment-plan.md) | Baseline and rollout plan. One-time Checkout/order/webhook hardening implemented in code; migration/test pending. |
| [`42-stripe-hardening-guide.md`](./42-stripe-hardening-guide.md) | Approval-gated implementation and combined dev test guide for one-time Stripe hardening. |
...
13. ~~**Stripe Checkout had no order/session idempotency**~~ — resolved. A unique active local order now deduplicates user/course/lifetime attempts; Checkout uses `checkout:<orderId>` as its Stripe idempotency key and concurrent calls reuse the same Session. Webhook fulfillment also deduplicates event, Session, PaymentIntent and entitlement source atomically. See [`30-flows-auth-payments.md`](./30-flows-auth-payments.md) → Flow 3 and [`42-stripe-hardening-guide.md`](./42-stripe-hardening-guide.md).
...
- Course/premium access is DB-authoritative through effective `courseEnrollments` grants. Navbar/Drawer and hardened payment fulfillment do not read or update Clerk course metadata.
