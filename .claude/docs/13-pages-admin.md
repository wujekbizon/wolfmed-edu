...
## `/admin/stripe-reports` - monthly Stripe reports

Read-only Stripe payments/refunds, previous completed month by default. Month boundaries and receipt timestamps use Europe/Warsaw. Receipt basis is Stripe balance entry creation; no bank payout-date substitution. Amounts are in Stripe settlement currency; original amounts, fees and invoice details are expandable. Missing historical billing data remains exportable. Existing invoices are linked, not created. Full balance reconciliation is excluded.

Server shell -> StripeReportContent -> getStripeReport -> Stripe SDK. Client React Query receives initial DTO; refresh uses authenticated GET. PDF download checks the preview fingerprint and returns 409 if data changed. No persistent PII/report storage. Test-mode exports carry a visible label.

The Gmail card starts admin-only OAuth with offline `gmail.send` access. The refresh token and sender email are AES-GCM encrypted in an HttpOnly cookie for the current admin browser. Sending re-fetches and verifies the report, emails the encrypted PDF, then sends its random password separately. Both messages use a formal responsive HTML template, embedded Wolfmed logo and plain-text fallback. Recipient defaults to `grzegorz.wolfinger@gmail.com` and remains editable.
