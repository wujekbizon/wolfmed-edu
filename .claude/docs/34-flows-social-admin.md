...
6. **Another user comments**: `createCommentAction` (`:610`) — rate-limited (20/hour), validated (`CreateCommentSchema`). Inside a transaction, checks **both** that the target post still exists **and** that it isn't `readonly` (an admin-lockable thread state) before inserting — a comment attempt on a locked or deleted thread fails with a specific message rather than silently succeeding or throwing a generic error.
7. **Author deletes their own post**: `deletePostAction` (`:572`) — checks `userId === authorId` from the submitted `authorId` field before deleting; a mismatch returns an error rather than deleting. (This check trusts the client-submitted `authorId` field to match the session's `userId` rather than re-fetching the post to verify ownership server-side — worth a closer look if hardening this path, though the comparison itself still requires the real session's `userId` to match, so it isn't an open ACL bypass.)
