...
`src/app/_components/Navbar.tsx` (`'use client'`) uses Clerk only for signed-in rendering. Navbar and NavDrawer expose navigation without a client ownership precheck; `/panel` and feature access remain server/DB-authoritative. `NavDrawer` uses `usePremiumAccess()` for DB-backed premium UI locks.
...
The actual source-aware entitlement is written later by the atomic Stripe fulfillment service — see [`14-api-routes.md`](./14-api-routes.md).
