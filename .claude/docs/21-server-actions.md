...
**3. Resolved: mind-map and manual-test validation use field-specific errors.** `generateMindMapAction` delegates Zod errors to `fromErrorToFormState`; `createTestAction` returns an empty top-level message with its `linkedCategory` error. Generic text no longer repeats beneath every field.
