...
| `deletePostAction` | `:572` | **Broken access control — see README audit note #9, which this pass corrects and escalates.** Reads *both* `postId` and `authorId` from client-submitted FormData, checks only `userId !== authorId`, then calls `deleteForumPost(postId)` — a query with **no ownership filter** (`DELETE FROM forumPosts WHERE id = ?`). The `postId` is never checked against the post's real author, so an attacker who submits their **own** `userId` as `authorId` (passing the check trivially) plus **any victim's** `postId` deletes that post. Any authenticated user can delete any forum post. |
...
| `deleteCommentAction` | `:681` | **Same broken access control as `deletePostAction`** — `commentId` + client-supplied `authorId`, compared only against the session `userId`, then `deleteForumComment(commentId)` with no ownership filter. Any authenticated user can delete any forum comment. |
...
**1. `deletePostAction` and `deleteCommentAction` allow any authenticated user to delete any forum post or comment.** This **corrects and escalates README audit note #9**, which previously described this as a stylistic looseness and explicitly concluded "the comparison itself can't be bypassed." That reasoning is right about the *comparison* but misses the actual hole: `postId`/`commentId` are also client-supplied and are **never** checked against the record's real owner. The underlying queries (`deleteForumPost`, `deleteForumComment`) filter on the id alone with no ownership predicate. An attacker submits their **own** `userId` as `authorId` — the `userId !== authorId` check passes trivially — together with **any** victim's `postId`, and the record is deleted. Fix is the pattern already used by `toggleConceptAction` in `planner.ts`: fetch the record server-side, compare its stored author to the session user, and scope the `DELETE ... WHERE` on both id and owner.
