...
| `submitTestAction` | `:196` | Rate-limited (`test:submit`). Accepts selected option indexes, reloads the authenticated session's deterministic question set, requires exactly one valid selection for every expected question, and grades against canonical DB answers; correctness is never sent by or trusted from the client. Error responses preserve exact selections and return both toast text and the same `general` field error below submit. Then in one transaction: re-fetches the session with a raw `SELECT ... FOR UPDATE` scoped to `(id, userId, status='ACTIVE')`, **re-checks `expiresAt` server-side**, updates aggregates, inserts `completedTestes`, and marks the session `COMPLETED`. Afterwards `after(() => onQuizCompleted(...))` recomputes memory facts. |
...
| `createTestAction` | `:806` | Manual single-question custom test creation. Validates the chosen `linkedCategory` against `getAccessibleCategories()` server-side (can't file a question under a course you don't own), and enforces **exactly one correct answer**. `LinkedSubjectSelect` has a matching field-error binding for schema and access failures. Every error response returns submitted string values so question, category, answers, and correct-answer selection survive validation. |
...
| `saveAIGeneratedTestsAction` | `:1004` | Persists Test-cell drafts and questions previewed by `generateAITestsAction` (`aiTests.ts`), links them to a custom category, and server-validates exactly one correct answer per question. |
| `expireSessionAction` | `:1095` | Plain function call (takes a `sessionId` string, not a form) — marks a session `EXPIRED` via `expireTestSession`. **Deliberately returns `SUCCESS` even on failure**, since this is a best-effort timer target. Browser hide/pagehide expiry uses the authenticated API route through `useBeaconCleanup`; heartbeat cleanup never expires sessions. |
...
| Function | Line | Behavior |
|---|---|---|
| `askRagQuestion` | `:142` | **The tutor chat entry point**. Commands/resources retain their existing priority. Unattached questions use semantic intent; self-state recall runs versioned canonical-event reconciliation and receives six bounded recent turns, while medical questions keep the existing RAG query/retrieval unchanged. Every free-form turn appends `user_msg` before model work, a retrieval event after recall, and `model_msg` after response. Source-required commands and the "no source, no output" gate remain unchanged. |
...
| Function | Behavior |
|---|---|
| `saveLectureInternal` | **Not a form action** — a plain internal helper called by `rag-actions.ts`'s `generateLectureAction`. Checks the storage quota **before** uploading anything, uploads the MP3 to UploadThing, then in one transaction lazily creates `userLimits`, adds `audioSize` to `storageUsed`, and inserts the lecture row with the same byte count in `lectures.size`. If the transaction fails, it deletes the just-uploaded file. |
| `deleteLectureAction` | `(lectureId) => FormState`. Rate-limited (`lecture:delete`). One ownership-scoped transaction deletes the lecture and refunds its stored `size` from `userLimits.storageUsed` with `GREATEST(0, ...)`, then deletes the UploadThing file. Existing pre-migration rows have `size: 0` and cannot receive a historical refund from row data alone. |
...
**2. Resolved: deleting an AI lecture refunds its storage quota.** New lecture rows persist the generated MP3 byte count in `lectures.size`. `deleteLectureAction` deletes the owned row and subtracts that size from `userLimits.storageUsed` in one transaction, floored at zero. Existing rows migrate with `size: 0`; their historical byte count was never stored and cannot be reconstructed from the database alone.
