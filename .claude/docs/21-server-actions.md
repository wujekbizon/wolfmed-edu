...
**3. Resolved: mind-map Zod validation uses field-specific errors.** `generateMindMapAction` delegates Zod errors to `fromErrorToFormState`. The reported `createTestAction` issue was a false positive: its category-access check is a business error and `LinkedSubjectSelect` has no `FieldError` binding, so it keeps a visible form-wide message like comparable actions in `actions.ts`.
