...
3. `generateTree(userId, topic)` (`src/lib/mindmap/generateTree.ts`) — the actual AI generation. Confirmed it does call `retrieveContext()` internally, the same way `generateAITestsAction` does (grounded generation, not raw model knowledge — consistent with root `CLAUDE.md`'s "No source, no output" rule).
