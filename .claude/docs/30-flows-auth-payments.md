...
4. Creates a Stripe Checkout Session (`mode: 'payment'`, Polish locale, tax ID collection, `client_reference_id: userId`, `metadata: { courseSlug, accessTier }`, `success_url=/success?session_id=...`, `cancel_url=/canceled`).
