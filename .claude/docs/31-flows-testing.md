...
3. Client navigates to `/panel/testy/[category]?sessionId=<id>`. The page verifies session ownership, category, active status and expiry, then derives a deterministic question and answer order from `sessionId`. It strips `isCorrect` before rendering `<GenerateTests />`, so correctness never reaches the browser.
