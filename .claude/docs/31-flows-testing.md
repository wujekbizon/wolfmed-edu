...
3. Client navigates to `/panel/testy/[category]?sessionId=<id>`. The page verifies session ownership/category; a completed session redirects server-side to `/panel/wyniki`, while invalid/expired sessions are rejected. An active session gets a deterministic question and answer order from `sessionId`. The page strips `isCorrect` before rendering `<GenerateTests />`, so correctness never reaches the browser.
4. **While the test is in progress**: two client hooks keep the session alive and honest:
   ...
   - `useBeaconCleanup(sessionId)` intentionally expires the session immediately on `document.hidden` (tab switch, minimize, OS backgrounding, phone lock) and `pagehide` (close/reload). This is anti-cheat behavior. A guarded delayed unmount also covers client navigation, but a same-session Strict Mode/Fast Refresh remount cancels that stale cleanup so a React lifecycle replay cannot end an otherwise active exam. `useSessionHeartbeat` never expires sessions.
   - `useCountdownTestTimer` counts down from the session's absolute server `expiresAt`, so reload/HMR cannot display more time than the server allows, and drives the low-time warning.
5. **Submission** (`submitTestAction`, `src/actions/actions.ts:196`): auth-checked, rate-limited (`test:submit`), requires `sessionId`. Each radio submits only its selected option index. The server reloads the deterministic session questions, rejects missing, extra, forged, non-numeric or out-of-range answers, and grades indexes against canonical DB answers. Client-provided correctness is never accepted.
   - Inside a DB transaction: re-fetches the session through typed Drizzle **`FOR UPDATE`**, scoped to `(id, userId, status='ACTIVE')`. Typed timestamp mapping keeps `expiresAt` as an absolute `Date`; raw timezone-less SQL strings must not be parsed in JavaScript.
   ...
   - Grades via `gradeSessionAnswers()` and stores only `{questionId, answer}` results.
...
**Files**: `src/actions/actions.ts` (`startTestAction`, `submitTestAction`), `src/app/panel/testy/[value]/page.tsx`, `src/components/{GenerateTests,TestCard}.tsx`, `src/helpers/{selectSessionTests,gradeSessionAnswers}.ts`, `src/server/testSessionQuestions.ts`, `src/hooks/useSessionHeartbeat.ts`, `src/hooks/useBeaconCleanup.ts`, `src/app/api/session/{heartbeat,expire}/route.ts`, `src/server/memory/extract.ts`.
