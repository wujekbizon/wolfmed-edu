...
import type { AIGenerationModalProps } from '@/types/generationModalTypes'

export default function AIGenerationModal({ title, statusMessages, hint }: AIGenerationModalProps) {
  ...
  useEffect(() => {
    ...
    // Generation is a single atomic call with no real sub-stages, so we ease
    // the bar toward ~92% and let it complete when the view unmounts on success.
    ...
    <BaseModal onClose={() => {}} size="sm">
      <div className="p-6 sm:p-8 flex flex-col items-center text-center gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <p className="text-sm text-zinc-400 min-h-[20px]">{statusMessages[messageIndex]}</p>
        </div>
