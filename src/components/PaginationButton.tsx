...
export default function PaginationButton({ disabled, onClick, children }: PaginationButtonProps) {
  return (
    <button
      ...
      } cursor-pointer min-w-24 stroke-neutral-400 stroke-[0.75] text-sm xs:text-base px-1 sm:px-3 py-1 text-zinc-900 transition-all rounded-md border  border-red-200/60 shadow shadow-zinc-500  hover:stroke-neutral-100 `}
    >
      {children}
    </button>
  )
