 ...
 */
export default function DiagramIconButton({ icon: Icon, label, theme, onClick }: DiagramIconButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      title={label}
      aria-label={label}
      className={`flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-lg transition-colors ${DIAGRAM_SURFACE[theme].button}`}
    >
