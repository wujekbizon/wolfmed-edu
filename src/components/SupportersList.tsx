...
import { Supporter } from '@/types/stripeTypes'

export default function SupportersList({supporters}:{supporters:Supporter[]}) {
  const visibleSupporters = useRotatingList<Supporter>(supporters, 6, 10000)
  ...
  return (
    <div className="grid grid-cols-2 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6 sm:gap-8">
      {visibleSupporters.map(({id,username}, index) => {
        let displayName = username

        if (typeof username === 'string' && username.includes('@')) {
          displayName = username.split('@')[0] as string
        }
