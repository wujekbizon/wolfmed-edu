export function buildStripeReportPdfEmailHtml(period: string): string {
  return `
<!doctype html>
<html lang="pl">
      ...
      <tr><td align="center">
        <table role="presentation" width="600" cellspacing="0" cellpadding="0" style="max-width:600px;width:100%;background:#fff;border:1px solid #e4e4e7;border-radius:16px;overflow:hidden">
          ...
          <tr><td style="padding:32px">
            <p style="margin:0 0 8px;color:#e11d48;font-size:12px;font-weight:700;letter-spacing:1px">Raport miesięczny</p>
            <h1 style="margin:0 0 24px;font-size:24px;line-height:1.3">Zestawienie transakcji Stripe</h1>
            <p style="margin:0 0 16px;line-height:1.6">Dzień dobry,</p>
            <p style="margin:0 0 20px;line-height:1.6">w załączniku przesyłam raport transakcji Stripe za <strong>${period}</strong>.</p>
            <div style="background:#fff1f2;border-left:4px solid #f43f5e;padding:14px 16px;line-height:1.5">Plik jest zabezpieczony hasłem. Hasło zostanie przesłane w osobnej wiadomości.</div>
