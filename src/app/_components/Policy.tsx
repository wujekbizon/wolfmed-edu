  ...
  return (
    <div className="h-full w-full max-w-4xl mx-auto bg-white flex flex-col p-4 sm:p-8 md:p-12 gap-4 rounded-2xl px-4 mb-8">
      <p className="text-base xs:text-lg sm:text-xl text-zinc-700 leading-7">
        Wolfmed Edukacja to innowacyjny startup edukacyjny, który oferuje bezpłatny dostęp do najnowszych testów i pytań
        medycznych, dedykowany przyszłym opiekunom medycznym przygotowującym się do egzaminów zawodowych. Dbamy o Twoją
        prywatność i chcemy, abyś czuł się pewnie, korzystając z naszych produktów oraz platformy. Usługi świadczy
        Wolfmed Edukacja (dalej jako „Wolfmed Edukacja”). Naszą polityką jest poszanowanie prywatności użytkowników i
        wszelkich informacji, które możemy zbierać podczas prowadzenia naszej strony internetowej.
      </p>
      ...
      <p className="text-base xs:text-lg text-zinc-800">
        ...
        odmówić dostarczenia informacji osobowych, co może uniemożliwić im korzystanie z wiekszości funkcji naszej
        aplikacji.
      </p>
      <p className="text-base xs:text-lg text-zinc-800">
      ...
      <h2 className="text-xl xs:text-2xl sm:text-3xl font-semibold mt-10 mb-3">Cookies</h2>
      <ul className="text-base xs:text-lg sm:text-xl text-zinc-700 leading-7 flex flex-col gap-4">
        ...
        przy każdej kolejnej wizycie. Wolfmed Edukacja korzysta z ciasteczek do identyfikacji i śledzenia odwiedzających
        oraz do zarządzania preferencjami dostępu do strony. Niektóre ciasteczka wygasają po pewnym czasie (session
        cookies), inne pozostają na urządzeniu przez dłuższy czas (persistent cookies). Nasza strona korzysta zarówno z
        ciasteczek pierwszej strony, jak i z ciasteczek osób trzecich, jak opisano poniżej:
        <li className="list-disc mx-8">
          Ściśle niezbędne pliki cookie: Używane do zapewnienia usług dostępnych na naszej stronie, w tym logowania i
          dostępu do zabezpieczonych obszarów. Te ciasteczka są dostarczane przez Clerk, nasz dostawca uwierzytelniania,
          i są niezbędne do korzystania z podstawowych funkcji strony. Nie można ich odmówić.
        </li>
        <li className="list-disc mx-8">
          Ciasteczka Analityczne/Wydajności: Używane do lepszego zrozumienia zachowań użytkowników i poprawy naszej
          strony. Nasza strona korzysta z Google Analytics, usługi analityki internetowej. Informacje zbierane przez
          Google (w tym adres IP) są przesyłane i przechowywane na ich serwerach. Czas przechowywania ciasteczek Google
          Analytics zależy od ich rodzaju; niektóre wygasają po sesji przeglądarki, inne mogą pozostawać do dwóch lat.
          Użytkownicy mogą zainstalować dodatek do przeglądarki, aby uniemożliwić zbieranie danych przez Google
          Analytics.
        </li>
      </ul>
      <p className="text-base xs:text-lg text-zinc-800">
      ...
      <p className="text-base xs:text-lg text-zinc-800">
        ...
        których korzystamy, takich jak Google Sign In, Recaptcha, Analytics oraz naszego dostawcy autoryzacji Clerk, a
        także na usługi Cloudflare, które są wykorzystywane do sprawdzania botów i zabezpieczeń.
      </p>
    </div>
  )
}
