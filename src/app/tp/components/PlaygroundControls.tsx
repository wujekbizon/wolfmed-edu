...
export default function PlaygroundControls({ events }: { events: Lecture[] }) {
  ...
  return (
    <>
      ...
      { (user?.publicMetadata.role === "teacher") && (<section className="bg-zinc-800 shadow-lg rounded-lg p-6 border border-zinc-700">
            ...
            >
              <div>
                <h3
                  className={clsx(
                    "font-medium",
                    event.status === "cancelled" && "line-through text-zinc-500"
                  )}
                >
                  {event.name}
                </h3>
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center">
          <div className="bg-zinc-800 rounded-lg p-6 max-w-md w-full border border-zinc-700">
            <h3 className="text-lg text-zinc-100 font-medium mb-4">
              Create New Lecture
            </h3>
            <CreateLectureForm />
          </div>
